...
# Templates: (source-class-filter, target-class-filter, label-pattern, segments)
# label-pattern uses {source} and {target} for substitution.
_TEMPLATES: list[tuple[ArchetypeClass | None, ArchetypeClass | None, str, int]] = [
    (ArchetypeClass.MOBILE, ArchetypeClass.TERRITORIAL,
     "{source} raid {target}", 4),
    (ArchetypeClass.MOBILE, ArchetypeClass.TERRITORIAL,
     "{source} extort tolls outside {target}", 6),
    (ArchetypeClass.TERRITORIAL, ArchetypeClass.TERRITORIAL,
     "{source} march on {target}", 8),
    (ArchetypeClass.TERRITORIAL, ArchetypeClass.MOBILE,
     "{source} hunt {target}", 6),
    (ArchetypeClass.EMBEDDED, None,
     "{source} schism the flock against {target}", 6),
    (ArchetypeClass.EMBEDDED, ArchetypeClass.TERRITORIAL,
     "{source} undermine {target} from within", 8),
    (ArchetypeClass.EMBEDDED, ArchetypeClass.EMBEDDED,
     "{source} expose {target} to the host", 4),
    (None, None,
     "{source} call in old debts on {target}", 4),
]
...
def _eligible_templates(
    ...
    half: list[tuple[str, int]] = []
    generic: list[tuple[str, int]] = []
    for src_filter, tgt_filter, pattern, segs in _TEMPLATES:
        ...
        if src_filter is not None and tgt_filter is not None:
            specific.append((pattern, segs))
        elif src_filter is None and tgt_filter is None:
            generic.append((pattern, segs))
        else:
            half.append((pattern, segs))
...
def seed_threats(factions: list[Faction], rng: random.Random) -> ThreatRegistry:
    ...
    for i, src in enumerate(factions):
        ...
        for j in candidate_indices:
            ...
            if not templates:
                continue
            pattern, segs = rng.choice(templates)
